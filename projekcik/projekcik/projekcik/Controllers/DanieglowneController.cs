...
namespace projekcik.Controllers
{
    ...
    {
        public ActionResult Index(Danieglowne dg)
        {
            List<Danieglowne> dania = new List<Danieglowne>()
            {
        ...
        public ActionResult AddCom(Danieglowne dg, Comment c)
        {
            List<Comment> komentarze = new List<Comment>();
            if (c.Nazwa != null)
            {
        ...
        [HttpPost]
        public ActionResult Create(Danieglowne danie)
        {
            if (ModelState.IsValid)
            {
                return RedirectToAction("Index", danie);
            }
            return View(danie);
        }
